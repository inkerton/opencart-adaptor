  ...
  try {
      ...
      return res.status(200).json({
        response: {
          ...
          error: {
            type: "REQUEST_ERROR",
            code: "10001",
            message: "Missing mandatory parameters in the /init request context.",
          },
        },
      });
    }
    ...
    setImmediate(async () => {
      try {
        ...
        // Send the signed request to the adapter's /on_init endpoint
        if (!process.env.ADAPTOR_SITE) {
          console.error("ADAPTOR_SITE environment variable is not set");
          return;
        }

        const adapterUrl = `${process.env.ADAPTOR_SITE.replace(/\/$/, '')}/api/on_init`;
    ...
    if (!res.headersSent) {
      res.status(500).json({ 
        error: {
          type: "CORE-ERROR",
          code: "50000",
          message: "An error occurred while processing the init request"
        }
      });
    }
  }
