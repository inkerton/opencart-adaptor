    ...
    try {
      ...
      setImmediate(async () => {
        try {
          ...
          // Send the signed request to the adapter's endpoint
          if (!process.env.ADAPTOR_SITE) {
            console.error("ADAPTOR_SITE environment variable is not set");
            return;
          }

          const adapterUrl = `${process.env.ADAPTOR_SITE}/api/on-incremental-catalog-refresh`;
          console.log("Sending request to adapter:", adapterUrl);

          await axios.post(
            // adapterUrl,
            `${process.env.ADAPTOR_SITE}/api/on-incremental-catalog-refresh`,
            ondcRequest,
            {
              headers: {
      ...
      if (!res.headersSent) {
        res.status(500).json({ 
          error: {
            type: "CORE-ERROR",
            code: "50000",
            message: "An error occurred while processing the incremental search request"
          }
        });
      }
    }
